...
def codenamify_words(word_list, contours):
    ...
    #rank coordinates of final contours by center point using quantile
...
def ocr(img):
    ...
    #process image and remove 'word noise'
...
class gameBoard:
    ...
    def getGameText(self, imgEncoding, use_b64_encoding=True):
        ...
        #convert b64 encoding to BGR and resize 
        ...
        dilatekernel = cv2.getStructuringElement(cv2.MORPH_RECT, (13,9))
        ...
        #apply closing operation to close gaps between characters
        ...
        #dilate remaining foreground objects to restore text regions
        dilate = cv2.dilate(erode, dilatekernel, iterations=4)
        ...
        for i, c in enumerate(orig_contours):
            ...
            line_items_coordinates.append([(int(boundRect[0]),int(boundRect[1])), (int(boundRect[0]+boundRect[2]), int(boundRect[1]+boundRect[3]))])


        #crop contours and convert to grayscale 
        ...
        #run tesseract in parallel to process set of cropped images
